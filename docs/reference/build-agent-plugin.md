...
| `manifest` | `null` | Attrset serialized to `plugin.json`. Required for an assembled package, whose src root files are not read; replaces the file of a passed-through tree (ADR 0008). |
| `skills` | `null` | Skill name → path in src. When null, the builder falls back to `skills-lock.json`, then to a passthrough tree. |
| `mcpServers` | `null` | Attrset serialized to `mcp.json`, declaring the same spec version as `manifest`; without a `manifest` it declares 1.0.0. An assembled package gets no `mcp.json` without it; a passed-through tree keeps its own unless this replaces it. |
