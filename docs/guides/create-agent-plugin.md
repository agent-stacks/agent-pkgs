...
Packages are generated rather than hand-written. `flox-agent
...
[flox-agent's import reference](https://github.com/flox/flox-agent/blob/main/docs/reference/import-command.md).
...
Nix with flakes, and git. You do not need `flox-agent` or `flox` installed.
The flake builds the importer on demand, which is what the
`nix run .#flox-agent` below does.

## 1. Import the upstream skills repository

```sh
# in the top-level directory of agent-pkgs
nix run .#flox-agent -- import flox/flox-skills -out pkgs
```
...
`flox-agent check-plugin` on its output as an install check, so
...
```sh
nix run .#flox-agent -- check-plugin ./result/share/agent-plugins/flox
```
...
is a known open question, recorded in flox-agent's
[ADR 0016](https://github.com/flox/flox-agent/blob/main/docs/decisions/0016-source-json-is-the-builder-call.md).
...
```sh
nix run github:agent-stacks/agent-pkgs#flox-agent -- \
...
| Import's flags and discovery rules | [flox-agent `import`](https://github.com/flox/flox-agent/blob/main/docs/reference/import-command.md) |
| Which deviations warn | [flox-agent `check-plugin`](https://github.com/flox/flox-agent/blob/main/docs/reference/check-plugin-command.md) |
